...
Begin VB.Form frmPurchase 
   ...
   Begin b8Controls4.b8TitleBar b8TB 
      ...
      _ExtentX        =   16219
      _ExtentY        =   661
      Caption         =   "Manage Purchase Entries"
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Arial"
         Size            =   12
         Charset         =   0
...
srcSQL = "SELECT qry_Purchase_Order.* " & _
        "FROM qry_Purchase_Order " & _
        "WHERE (((" & cboFields.Text & ") Like '%" & txtSearch.Text & "%'))"

Set RS_PURCHASE = New ADODB.Recordset
If RS_PURCHASE.State = adStateOpen Then RS_PURCHASE.Close
RS_PURCHASE.Open srcSQL, CN, adOpenDynamic, adLockOptimistic

If txtSearch.Text = vbNullString Then txtSearch.SetFocus: Exit Sub

If RS_PURCHASE.RecordCount < 1 Then
...
MAIN.AddToWin "Purchase Entries", Name
...
srcSQL = "SELECT qry_Purchase_Order.* " & _
            " FROM qry_Purchase_Order " & _
            " ORDER BY qry_Purchase_Order.PurchaseOrderID ASC "

Set RS_PURCHASE = New ADODB.Recordset
If RS_PURCHASE.State = adStateOpen Then RS_PURCHASE.Close
RS_PURCHASE.Open srcSQL, CN, adOpenDynamic, adLockPessimistic
...
On Error Resume Next
srcPurchase = lvList.SelectedItem.Index
srcRecord = lvList.ListItems.Item(srcPurchase).Text
Call RefreshRecSum
End Sub
...
Select Case srcPerformWhat
    ...
    Case "Delete" 'Delete
            ...
            If MsgBox("Are you sure you want to delete this record?", vbCritical + vbYesNo) = vbYes Then
                sSQL_Delete "DELETE FROM Purchase_Order WHERE PurchaseOrderID=" & srcRecord
...
With lvList
    ...
    .ColumnHeaders.Add , , "SupplierName"
    .ColumnHeaders.Add , , "Address"
    .ColumnHeaders.Add , , "TelNo"
    ...
    .ListItems.Clear
    Do While Not RS_PURCHASE.EOF
    Set srcItem = .ListItems.Add(, , RS_PURCHASE.Fields("PurchaseOrderID"), 1, 1)
        srcItem.SubItems(1) = RS_PURCHASE.Fields("SupplierID")
        srcItem.SubItems(2) = RS_PURCHASE.Fields("SupplierName")
        srcItem.SubItems(3) = RS_PURCHASE.Fields("Address")
        srcItem.SubItems(4) = RS_PURCHASE.Fields("TelNo")
        srcItem.SubItems(5) = RS_PURCHASE.Fields("Salesman")
        srcItem.SubItems(6) = RS_PURCHASE.Fields("Date")
        srcItem.SubItems(7) = toMoney(RS_PURCHASE.Fields("Gross"))
        srcItem.SubItems(8) = toNumber(RS_PURCHASE.Fields("Discount"))
        srcItem.SubItems(9) = toMoney(RS_PURCHASE.Fields("NetAmount"))
        srcItem.SubItems(10) = RS_PURCHASE.Fields("StatusID")
        srcItem.SubItems(11) = RS_PURCHASE.Fields("StatusDesc")
        srcItem.SubItems(12) = RS_PURCHASE.Fields("Remarks")
...
Set RS_PURCHASE = New ADODB.Recordset
If RS_PURCHASE.State = adStateOpen Then RS_PURCHASE.Close
RS_PURCHASE.Open jSQL, CN, adOpenDynamic, adLockOptimistic
...
Private Sub RefreshRecSum()
    lblRecSum.Caption = "Record: " & srcPurchase & " of " & lvList.ListItems.Count
End Sub
