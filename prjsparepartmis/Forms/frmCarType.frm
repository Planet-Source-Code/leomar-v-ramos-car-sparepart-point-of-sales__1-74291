...
RS_CARTYPE.Open srcSQL, CN, adOpenDynamic, adLockOptimistic

If txtSearch.Text = vbNullString Then Exit Sub:

If RS_CARTYPE.RecordCount < 1 Then
...
srcSQL = "SELECT Car_Types.* " & _
            " FROM Car_Types " & _
            " ORDER BY Car_Types.CarTypeID ASC "

Set RS_CARTYPE = New ADODB.Recordset
If RS_CARTYPE.State = adStateOpen Then RS_CARTYPE.Close
RS_CARTYPE.Open srcSQL, CN, adOpenDynamic, adLockPessimistic
...
On Error Resume Next
srcCarModel = lvList.SelectedItem.Index
srcRecord = lvList.ListItems.Item(srcCarModel).Text
Call RefreshRecSum
End Sub
...
Select Case srcPerformWhat
    ...
    Case "Delete" 'Delete
            ...
            If isRecordExist("Spare_Parts", "CarTypeID", srcRecord, True) = True Then
            ...
            Else
                If MsgBox("Are you sure you want to delete this record?", vbCritical + vbYesNo) = vbYes Then
                    sSQL_Delete "DELETE FROM Car_Types WHERE CarTypeID='" & srcRecord & "'"
                    
                    Kill (App.Path & "\Graphics\Car Types\" & srcRecord & ".img")
...
With lvList
    ...
    .ListItems.Clear
    Do While Not RS_CARTYPE.EOF
    Set srcItem = .ListItems.Add(, , RS_CARTYPE.Fields("CarTypeID"), 1, 1)
        srcItem.SubItems(1) = RS_CARTYPE.Fields("CarTypeName")
        srcItem.SubItems(2) = RS_CARTYPE.Fields("MakeID")
        srcItem.SubItems(3) = RS_CARTYPE.Fields("Remarks")
    RS_CARTYPE.MoveNext
    Loop
End With
End Sub
...
Private Sub RefreshRecSum()
    lblRecSum.Caption = "Record: " & srcCarModel & " of " & lvList.ListItems.Count
End Sub
